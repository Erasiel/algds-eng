...
def is_balanced(node: BinaryTreeNode) -> bool:
    # We'll use the correct height implementation from Exercise 03
    from exercise_03 import height

    # The empty subtree is balanced
    if node is None: return True

    # Get the heights of the two children
    height_left = height(node.left)
    height_right = height(node.right)

    # Check if the current node is balanced
    current_is_balanced = abs(height_left - height_right) <= 1

    # Check if every node is balanced in the left subtree recursively with
    # `is_balanced(node.left)` and similarly in the right subtree with
    # `is_balanced(node.right)`
    return (current_is_balanced and
