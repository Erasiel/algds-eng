...
def num_leaf_nodes(node: BinaryTreeNode) -> int:
    ...
    # An empty subtree contains no leaf nodes
    if node is None:
        return 0

    # If the subtree is a leaf node, return 1
    if node.left is None and node.right is None:
        return 1

    # Return the sum of the leaf nodes in the left and the right subtrees
    return num_leaf_nodes(node.left) + num_leaf_nodes(node.right)
