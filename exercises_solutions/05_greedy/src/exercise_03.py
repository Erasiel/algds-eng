...
def car_trip(n: int, distances: List[int]) -> int:
    # Greedy choice: go as far as possible without refueling, only refuel if
    # the next section of the trip cannot be covered without refueling.
    ...
    for next_distance in distances:
        ...
        if remaining_fuel < next_distance:
            remaining_fuel = n
            n_stops += 1

        # Travel this part of the trip
        remaining_fuel -= next_distance
