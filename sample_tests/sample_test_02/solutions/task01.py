...
def num_nodes_at_depth(root: BinaryTreeNode, k: int) -> int:
    # NIL node (empty subtree)
    if root is None:
        return 0

    # The node is at the required depth
    if k == 0:
        return 1

    # Count the number of nodes at depth k in the left and right subtrees
    # separately. Since the left and right children are at a lower depth,
    # decrease the passed k value by one.
    return (num_nodes_at_depth(root.left, k - 1) +
            num_nodes_at_depth(root.right, k - 1))
